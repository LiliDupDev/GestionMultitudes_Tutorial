...
* Name: Tutorial05
* Author: Lili
* Description: Graficas para analizar comportamientos
***/
...
global
{
	...
	int		people				<- 1000		category:'Scenario';
	...
	reflex count_interactions when:every(1#minute){
		count_interactions <- (person sum_of length(each.mobility_contact));
	}
...
species person skills:[moving]{
	...
	list<person> 		mobility_contact <-[] update:person at_distance(health_distance#meters); 
	...
	output{
		...
		display Statistics
		{
        	...
        	chart "People count" type: histogram  size: {0.5,0.5} position: {0, 0}{
	        	datalist store_point value:(store_point collect each.current_people) legend:(store_point collect each.store) color:(store_point collect each.color_i);
	        }
			...
			{
				datalist store_point value:(store_point collect each.crowd_percentage) legend:(store_point collect each.store) color:(store_point collect each.color_i) marker:false;
